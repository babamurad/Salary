...
begin
  ...
  begin
    ...
    begin
      ...
      Alignment := taCenter;
      Width := 80;
      ReadOnly := True; // Запрет редактирования именно в гриде — само поле
                        // остаётся доступным для записи из кода (см. ниже)
    end;
    with DBGridTimesheet.Columns.Add do
    begin
    ...
    // Норма — справочная. Редактирование запрещено на уровне колонки грида
    // (Column.ReadOnly выше), а не самого поля — иначе код ниже, который
    // сам заполняет эти значения (FillEmployeesList/btnAutoFillClick), тоже
