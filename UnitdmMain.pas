...
begin
  try
    ...
    // точности, а не как Currency (в SQLite эти же колонки были объявлены
    // типом CURRENCY, и FireDAC-драйвер SQLite узнавал это имя напрямую).
    // Старый код и persistent-поля датасетов ждут именно Currency — без
