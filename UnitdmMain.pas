...
type
  TdmMain = class(TDataModule)
    ...
    qryEmployeeswork_fraction: TFloatField;
    qryEmployeesis_tax_exempt: TIntegerField;
    qryEmployeesclass_rank: TIntegerField;
    qryEmployeestrade_union: TIntegerField;
    qryEmployeesalimony_percent: TFloatField;
...
// Таблица появилась уже после того, как многие базы (в том числе
// демо-база и рабочие базы у пользователей) были созданы — досоздаём её
// автоматически при каждом подключении (CREATE TABLE IF NOT EXISTS,
// безопасно выполнять повторно), чтобы не заставлять никого вручную
// выполнять миграцию. Хранит детализацию начислений/удержаний по каждой
// строке payroll_journal — нужна для расчётного листка с разбивкой.
...
begin
  conn.ExecSQL(
    ...
    '  details TEXT,' +                 // произвольное пояснение (часы, вычеты и т.п.)
    '  base_amount CURRENCY,' +         // база, от которой считался % (если применимо)
...
// Входные данные, с которыми считалась конкретная строка payroll_journal —
// нужны, чтобы форма детализации могла честно ПЕРЕСЧИТАТЬ начисление после
// правки бухгалтером (часы, оклад, надбавки, удержания), а не просто менять
// готовую сумму вручную. По одной строке на каждую строку payroll_journal.
...
begin
  Result := 0;
  // Отступаем ровно на 12 месяцев назад от 1-го числа месяца расчета
  StartDate := FormatDateTime('yyyy-mm-dd', IncMonth(StartOfTheMonth(ACalcDate), -12));
  // Верхняя граница - 1-е число месяца ухода в отпуск (текущий месяц не берем)
  ...
  try
    ...
    // Если парсер FireDAC всё еще слепой, создаем параметры ПРИНУДИТЕЛЬНО
    ...
    // Передаем значения (они автоматически разлетятся по всем местам в SQL-запросе)
